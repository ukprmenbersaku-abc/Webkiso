...
let newContent = constantsContent.slice(0, jsLogicLessonsStart) + beginnerJS + constantsContent.slice(jsLogicLessonsEnd);

// I will attempt a clean close at the very end to fix the balance
const lastArrayClose = newContent.lastIndexOf('];');
if (lastArrayClose !== -1) {
    // Check if there are extra brackets/braces after it
    const after = newContent.slice(lastArrayClose);
    // Standard ending should be ];\n
    newContent = newContent.slice(0, lastArrayClose) + '];\\n';
}
