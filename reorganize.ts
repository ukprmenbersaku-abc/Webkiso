...
// 1. Define Correct Beginner JS Lessons
...
// 2. Extract 15 Game Lessons
// We'll search for the current misplacement and extract them.
const gameStartMarker = "id: 'game-layout-grid'";
const jsDomTextMarker = "id: 'js-dom-text'";
const gameStartIndex = constantsContent.indexOf('{', constantsContent.indexOf(gameStartMarker) - 20);
const gameEndIndex = constantsContent.lastIndexOf('}', constantsContent.indexOf(jsDomTextMarker) - 5) + 1;
const gameLessonsPart = constantsContent.slice(gameStartIndex, gameEndIndex);

// 3. Assemble new content
const jsLogicStart = constantsContent.indexOf("id: 'js-logic'");
const jsLogicLessonsStart = constantsContent.indexOf('lessons: [', jsLogicStart) + 10;
const jsLogicLessonsEnd = constantsContent.lastIndexOf(']', constantsContent.indexOf("id: 'css-layout'")) + 1;

const advancedModuleStart = constantsContent.indexOf("id: 'web-advanced'");
const advancedLessonsStart = constantsContent.indexOf('lessons: [', advancedModuleStart) + 10;
const advancedLessonsEnd = constantsContent.lastIndexOf(']', constantsContent.indexOf(']', advancedLessonsStart) + 10); // Find the closing bracket of the lessons array

// Find the end of the entire module array
const finalClosingBracket = constantsContent.lastIndexOf('];');

let newContent = constantsContent.slice(0, jsLogicLessonsStart);
newContent += beginnerLessons;
newContent += constantsContent.slice(jsLogicLessonsEnd, advancedLessonsStart);
newContent += gameLessonsPart;
newContent += constantsContent.slice(advancedLessonsEnd);
